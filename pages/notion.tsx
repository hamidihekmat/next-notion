import { GetStaticProps, InferGetStaticPropsType, NextPage } from 'next';
import { notion } from 'lib/notion';
import { NotionRenderer, Code } from 'react-notion-x';
import { useEffect } from 'react';
import toast from 'react-hot-toast';

const copyToClipboard = () =>
  toast.success('successfully copied code snippet', {
    position: 'top-right',
    style: {
      background: 'inherit',
      color: 'whitesmoke',
      outline: '1px solid #4A5568',
    },
  });

export const getStaticProps: GetStaticProps = async () => {
  const recordMap = await notion.getPage('95f6917260374711b2397c666c4e1e0a');
  let codeBlocks = [];
  for (let block in recordMap.block) {
    if (recordMap.block[block].value.type === 'code') {
      codeBlocks.push(recordMap.block[block].value.properties?.title[0]);
    }
  }
  return {
    props: {
      recordMap,
      codeBlocks,
    },
    revalidate: 10,
  };
};

const Notion: NextPage<InferGetStaticPropsType<typeof getStaticProps>> = ({
  recordMap,
  codeBlocks,
}) => {
  useEffect(() => {
    if (codeBlocks) {
      // hacky way of manipulating code component
      const codes = document.getElementsByClassName('notion-code');
      Array.from(codes).forEach((code, index) => {
        code.classList.add('relative');
        const clipBoard = document.createElement('div');
        clipBoard.addEventListener('click', () => {
          navigator.clipboard.writeText(codeBlocks[index]);
          copyToClipboard();
        });
        clipBoard.innerHTML = `
            <div class='clipboard'>
                <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 9.50006C1 10.3285 1.67157 11.0001 2.5 11.0001H4L4 10.0001H2.5C2.22386 10.0001 2 9.7762 2 9.50006L2 2.50006C2 2.22392 2.22386 2.00006 2.5 2.00006L9.5 2.00006C9.77614 2.00006 10 2.22392 10 2.50006V4.00002H5.5C4.67158 4.00002 4 4.67159 4 5.50002V12.5C4 13.3284 4.67158 14 5.5 14H12.5C13.3284 14 14 13.3284 14 12.5V5.50002C14 4.67159 13.3284 4.00002 12.5 4.00002H11V2.50006C11 1.67163 10.3284 1.00006 9.5 1.00006H2.5C1.67157 1.00006 1 1.67163 1 2.50006V9.50006ZM5 5.50002C5 5.22388 5.22386 5.00002 5.5 5.00002H12.5C12.7761 5.00002 13 5.22388 13 5.50002V12.5C13 12.7762 12.7761 13 12.5 13H5.5C5.22386 13 5 12.7762 5 12.5V5.50002Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
                <p style='margin: 0px 5px;'>Clipboard Copy</p>
            </div>
    
      `;
        code.appendChild(clipBoard);
      });
    }
  }, [codeBlocks]);
  return (
    <>
      <NotionRenderer
        recordMap={recordMap}
        fullPage={true}
        components={{ code: Code }}
        darkMode
      />
    </>
  );
};

export default Notion;
